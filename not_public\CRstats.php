...
$time = time();
$S_mapNumber = 0;
$sql = "SELECT online,  lastaction, loggedin  FROM stats WHERE loggedin<>'' && online>0 ORDER BY online desc";
$resultaat = mysqli_query($mysqli, $sql);
while ($record = mysqli_fetch_object($resultaat))
{
    $totalonline = $totalonline + $record->online;
    $lastaction = $lastaction + $record->lastaction;
    $lastlogin = $lastlogin + $record->loggedin;
    $S_mapNumber = $S_mapNumber + 1;
}

$nr = 0;
$sql = "SELECT joined, gold, totalskill  FROM users WHERE joined<>''";
$resultaat = mysqli_query($mysqli, $sql);
while ($record = mysqli_fetch_object($resultaat))
{
    $totalskill = $record->totalskill + $totalskill;
    #$age=$age+$record->age;
    $gold = $gold + $record->gold;
    $joined = $joined + $record->joined;
    $nr = $nr + 1;
}

$sql = "SELECT gold  FROM buildings";
$resultaat = mysqli_query($mysqli, $sql);
while ($record = mysqli_fetch_object($resultaat))
{
...
$avgjoin = round($time - ($joined / $nr));
$avggold = round(($gold / $nr));
...
$resultaat = mysqli_query($mysqli,$sql);     
while ($record = mysqli_fetch_object($resultaat)) { $currentID=$record->ID; }
...
echo "All Syrnia players that are still registered have spend a total of $hoursleft hours and $minleft minutes playing syrnia since 11-3-2005, and got $gold gold.<BR>";
...
$resultaat = mysqli_query($mysqli,
    "SELECT COUNT(*) as totaal FROM users WHERE joined<($time-86400) && joined>($time-86400*2)");
$totaal = $resultaat->fetch_assoc();
$aantal2 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT COUNT(*) as totaal  FROM users WHERE  joined<($time-86400*2) &&  joined>($time-86400*3)");
$totaal = $resultaat->fetch_assoc();
$aantal3 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT COUNT(*)  as totaal FROM users WHERE joined<($time-86400*3) && joined>($time-86400*4)");
$totaal = $resultaat->fetch_assoc();
$aantal4 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT COUNT(*) as totaal  FROM users WHERE joined<($time-86400*4) && joined>($time-86400*5)");
$totaal = $resultaat->fetch_assoc();
$aantal5 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT COUNT(*) as totaal  FROM users WHERE joined<($time-86400*5) && joined>($time-86400*6)");
$totaal = $resultaat->fetch_assoc();
$aantal6 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT COUNT(*) as totaal FROM users WHERE joined<($time-86400*6) && joined>($time-86400*7)");
$totaal = $resultaat->fetch_assoc();
...
$resultaat = mysqli_query($mysqli,
    "SELECT  COUNT(*)  as totaal FROM users WHERE joined<($time) && joined>($time-604800)");
$totaal = $resultaat->fetch_assoc();
$aantal1 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT  COUNT(*) as totaal  FROM users WHERE joined<($time-604800) && joined>($time-604800*2)");
$totaal = $resultaat->fetch_assoc();
$aantal2 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT  COUNT(*) as totaal  FROM users WHERE  joined<($time-604800*2) &&  joined>($time-604800*3)");
$totaal = $resultaat->fetch_assoc();
$aantal3 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT  COUNT(*) as totaal  FROM users WHERE joined<($time-604800*3) && joined>($time-604800*4)");
$totaal = $resultaat->fetch_assoc();
$aantal4 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT  COUNT(*)  as totaal FROM users WHERE joined<($time-604800*4) && joined>($time-604800*5)");
$totaal = $resultaat->fetch_assoc();
$aantal5 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT  COUNT(*)  as totaal FROM users WHERE joined<($time-604800*5) && joined>($time-604800*6)");
$totaal = $resultaat->fetch_assoc();
$aantal6 = $totaal['totaal'];
$resultaat = mysqli_query($mysqli,
    "SELECT  COUNT(*)  as totaal FROM users WHERE joined<($time-604800*6) && joined>($time-604800*7)");
$totaal = $resultaat->fetch_assoc();
$aantal7 = $totaal['totaal'];
...
if ($COUNTRY == 111)
{
	...
	<tr><Td>Rank<td>Country<td>Players<td>%";
    $rank = '1';
...
<tr><td>Rank<td>Seconds played<td>Player<td>%";
$rank = '1';
